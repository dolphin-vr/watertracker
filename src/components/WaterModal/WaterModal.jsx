...
export const WaterModal = ({ title, subtitle, onCloseModal, onAddWater, onEditWater, initialWater, initialDate, initialId, isEditing }) => {
  ...
  const handleSave = values => {
    ...
    const contextData = isEditing ? { id: initialId, portion: { ...waterData } } : waterData;
  ...
  const validationSchema = Yup.object().shape({
    waterCounter: Yup.number().min(1, "Min 1 ml").max(5000, "Max 5000 ml").required("Enter water value"),
    date: Yup.date(),
  });
      ...
      <ModalWrap initialWater={initialWater}>
        ...
        {initialWater !== 0 && (
          <Container>
            ...
            <Time>{initialDate && <span>{` ${initialDate.getHours().toString().padStart(2, "0")}:${initialDate.getMinutes().toString().padStart(2, "0")}`}</span>}</Time>
          </Container>
        )}
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          <label>
            <WrapInput>
              ...
              />
              {formik.touched.waterCounter && formik.errors.waterCounter && <ErrorWrap>{formik.errors.waterCounter}</ErrorWrap>}
            </WrapInput>
          </label>
